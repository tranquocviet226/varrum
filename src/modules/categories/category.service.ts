...
export class CategoryService {
    ...
    async findAll(pagination: PaginationRequest<QueryRequest>): Promise<PaginationResponseDto<CategoryEntity>> {
        try {
    ...
    async create(createDto: CreateCategoryDto) {
        const photoEntity = await this.photoRepository.findOne({ id: createDto.photo_id })
        if (createDto.photo_id && !photoEntity) throw new CommonException(ErrorType.IMAGE_NOT_FOUND, ErrorMessage.IMAGE_NOT_FOUND)
    ...
    async update(updateDto: UpdateCategoryDto) {
        ...
        if (!category) throw new CommonException(ErrorType.ID_NOT_FOUND, ErrorMessage.ID_NOT_FOUND)
        const photoEntity = await this.photoRepository.findOne({ id: updateDto.photo_id })
        if (updateDto.photo_id && !photoEntity) throw new CommonException(ErrorType.IMAGE_NOT_FOUND, ErrorMessage.IMAGE_NOT_FOUND)
        try {
